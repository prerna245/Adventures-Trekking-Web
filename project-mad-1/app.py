...
def register():
    if request.method == "POST":
        ...
        adv_users = AdventureUser(usrname=name ,usr_mail=email,state_of_usr=state,city_of_usr=city,county=country, phone_no=ph_number,any_experince=experience,
        any_medical_isue=medical_issue,rolee=role,pasword=hashed_password,staf_satus="Pending" if role == "staff" else "Approved")
...
def add_trek():
    ...
    if request.method == "POST":
        ...
        rout_price = int(request.form.get("rout_price"))
        capcity = int(request.form.get("capcity"))
        remening_spots = int(request.form.get("remening_spots"))
        strt_trck_date = request.form.get( "strt_trck_date" )
        end_trck_date = request.form.get( "end_trck_date" )
        distnce = int(request.form.get("distnce"))
        max_altitdu = int(request.form.get("max_altitdu"))
        ...
        if remening_spots < 0:
            return "remaning slot can be 0 not in negative"
        if remening_spots > capcity:
            return "remaining slot can't greater than toal capacity "
        if rout_price < 0:
            return "route price can't be negative"
        if distnce <= 0:
            return "distance must be greater than 0."
        if max_altitdu <= 0:
            return "maximum altitude must greater than 0"
        if strt_trck_date < date.today():
            return "start date must be from today onwards"
        if end_trck_date < strt_trck_date:
            return "Start of trek can't after ending trek"
        actual_days = (end_trck_date - strt_trck_date).days + 1
        
        # Check if staff is already assigned to another trek
